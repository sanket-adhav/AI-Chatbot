...
def run():
    ...
    r = requests.post(f"{base}/auth/register", json={"username": "tester999", "email": "tester999@test.com", "password": "password"})
    if r.status_code == 400:
        print("Already registered, logging in...")
        r = requests.post(f"{base}/auth/login", data={"username": "tester999@test.com", "password": "password"})
